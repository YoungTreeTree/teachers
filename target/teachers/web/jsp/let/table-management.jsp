                ...
                <ul class="nav navbar-nav">
                    <li class="user user-menu">
                        <a href="javascript:;" class="dropdown-toggle">
                            <span class="hidden-xs">sc ${userself.uCodeQ}</span>
                        </a>
                    </li>
                    <li class="user user-menu">
    ...
    <aside class="main-sidebar">
        <section class="sidebar">
            <ul class="sidebar-menu" data-widget="tree">
                <li ><a href="${pageContext.request.contextPath}/t/management"><span>单位管理</span></a></li>
                <li class="active"><a href="${pageContext.request.contextPath}/t/-1/-1/table-management"><span>表格管理</span></a></li>
                <li><a href="${pageContext.request.contextPath}/t/-1/table-list"><span>汇总统计</span></a></li>
                <li ><a href="${pageContext.request.contextPath}/t/psd"><span>修改密码</span></a></li>
            </ul>
        </section>
    </aside>
                        ...
                        <div class="box-header with-border">
                            ...
                            <select class="form-control" id="codes">
                                <c:forEach items="${users}" var="user"    varStatus="id">
                                    <option value="${user.uId}"> ${user.uCodeW} ${user.uV2Name}</option>
                                </c:forEach>
                                <option value="-1">全部</option>
                            </select>
                        ...
                        <div class="box-body">
                            <table class="table table-bordered">
                                ...
                                <c:forEach items="${tables}" var="item"    varStatus="id">
                                    <tr>
                                        <td>${item.tableCopyCopy.tUserInputYear}</td>
                                        <td>${item.user.uV2Name}</td>
                                        <td>st${item.user.uCodeW}</td>
                                        <td>
                                            <c:if test="${item.tableCopyCopy.tIfQOk==1}"> -- </c:if>
                                            <c:if test="${item.tableCopyCopy.tIfQOk==2}"> 通过 </c:if>
                                            <c:if test="${item.tableCopyCopy.tIfQOk==3}"> 被驳回 </c:if></td>
                                        <td>
                                            <a href="${pageContext.request.contextPath}/common/${item.tableCopyCopy.tId}/statics2" target="_blank" class="btn btn-default">查看详情</a>
                                            <c:if test="${item.tableCopyCopy.tIfQOk==1}">
                                                <a href="javascript:;" data-toggle="modal" data-target="#deny-alert"  class="btn btn-default" onclick="f2(${item.tableCopyCopy.tId})" >驳回</a>
                                                <a href="javascript:;" data-toggle="modal" data-target="#up-alert"  class="btn btn-default" onclick="f3(${item.tableCopyCopy.tId})" >通过</a>
                                            </c:if>

                                        </td>
                                        <td>
                                            <c:if test="${item.tableCopyCopy.tIfQOk==1}">
                                                <input type="checkbox" class="table-checkbox" data-id="${item.tableCopyCopy.tId}"/>
                                            </c:if>
                                        </td>
                                    </tr>
                                </c:forEach>
...
<script>
    ...
    function f() {
        var url = '${pageContext.request.contextPath}/t/'+document.getElementById('years').value+'/'+document.getElementById('codes').value+'/table-management';
        window.location.href=url;
    }


    function f2(code) {
        document.getElementById('cancel_form').setAttribute("action",'${pageContext.request.contextPath}/t/'+code+'/cancel');
    }
    function f3(code) {
        document.getElementById('cancel_form').setAttribute("action",'${pageContext.request.contextPath}/t/'+code+'/up');
    }
    function  cancel() {
...
<script>
    $("#submit-all-id").on("click",function () {
        ...
        if(idString==''){
            ...
            var url = '${pageContext.request.contextPath}/t/'+idString+'/up_group';
            window.location.href=url;
        }
