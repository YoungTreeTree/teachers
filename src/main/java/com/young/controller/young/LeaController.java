...
import com.young.services.le_a.impl.LeaServiceImpl;
...
@Controller
@RequestMapping(value = "/lea")
public class LeaController extends BaseController {
    @Resource
    ...
    public ModelAndView table() throws Exception {
        ...
        TableCopy tableCopy = leaService.get_table_u_id(user.getuId());
    ...
    public ModelAndView history() throws Exception {
        ...
        modelAndView.getModel().put("user",user);
        ArrayList<TableVo> tables = leaService.get_tables_u_id(user.getuId());
    ...
    public ModelAndView detail(
        ...
        modelAndView.getModel().put("user",user);
        TableCopy tableCopy = leaService.get_table_by_id(t_id);
        String reason = "";
    ...
    public ModelAndView up_table(
        ...
        queryJson  = leaService.up_table(p,user.getuId(),if_temp, year);
        ...
        modelAndView.getModel().put("url","/lea/history");
    ...
    public ModelAndView change_pw(
        ...
        if (!new_pw1.equals(new_pw)){
            ...
            modelAndView.getModel().put("url","/lea/psd");
            modelAndView.setViewName("return");
            return modelAndView;
        }
        User user =getUserSession();
        ...
        int flag = leaService.change_pw2(old_pw,new_pw,user.getuId(),name,phone,mail);
        if (flag==1){
