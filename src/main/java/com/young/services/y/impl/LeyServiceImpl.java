package com.young.services.y.impl;
...
@Service(value = "leyService")
public class LeyServiceImpl extends CommServiceImpl implements ILeyService {
    ...
    public BaseJson up_table(String[] p, long u_id, int if_temp,String year) {
        ...
        try {
            ...
            ArrayList<TableCopyCopy> tables = (ArrayList<TableCopyCopy>)baseDAO.findByProperties(map,TableCopyCopy.class);

            if (tables.size()<=0){
                TableCopyCopy table = new TableCopyCopy();
                table =  change_table(table,p,1,u_id,year);
                baseDAO.save(table);
                queryjson.setErrno("1");
                queryjson.setErrmsg("成功保存");
            }else{
                TableCopyCopy table = tables.get(0);
    ...
    @Override
    public TableCopyCopy get_table_u_id(long u_id) {
        Map map = new HashMap();
        map.put("uId",u_id);
    ...
    private TableCopyCopy change_table(TableCopyCopy table, String p[], int if_temp, long u_id,String year){
    ...
    public ArrayList<TableVo> get_tables_u_id(long u_id) {
        ArrayList<TableCopyCopy> tables = (ArrayList<TableCopyCopy>)baseDAO.findByProperty("uId",u_id,TableCopyCopy.class);
        ArrayList<TableVo> re_tables = new ArrayList<>();
        for (TableCopyCopy t:tables
             ) {
