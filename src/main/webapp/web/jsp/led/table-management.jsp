                ...
                <ul class="nav navbar-nav">
                    <li class="user user-menu">
                        <a href="javascript:;" class="dropdown-toggle">
                            <span class="hidden-xs">${userself.uCodeQ} ${userself.uName}</span>
                        </a>
                    </li>
                    <li class="user user-menu">
    ...
    <aside class="main-sidebar">
        <section class="sidebar">
            <ul class="sidebar-menu" data-widget="tree">
                <li ><a href="${pageContext.request.contextPath}/d/management"><span>单位管理</span></a></li>
                <li class="active"><a href="${pageContext.request.contextPath}/d/-1/-1/table-management"><span>表格管理</span></a></li>
                <li><a href="${pageContext.request.contextPath}/d/-1/table-list"><span>汇总统计</span></a></li>
                <li ><a href="${pageContext.request.contextPath}/d/psd"><span>修改密码</span></a></li>
            </ul>
        </section>
    </aside>
                        ...
                        <div class="box-header with-border">
                            ...
                            <select class="form-control" id="codes">
                                <c:forEach items="${users}" var="user"    varStatus="id">
                                    <option value="${user.uId}">${user.uV2Name} ${user.uV2Phone}</option>
                                </c:forEach>
                                <option value="-1">全部</option>
                            </select>
                                ...
                                <c:forEach items="${tables}" var="item"    varStatus="id">
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href="${pageContext.request.contextPath}/common/${item.tableCopy.tId}/word" target="_blank" class="btn btn-default">导出word打印</a>
...
<script>
    ...
    function f() {
        var url = '${pageContext.request.contextPath}/d/'+document.getElementById('years').value+'/'+document.getElementById('codes').value+'/table-management';
        window.location.href=url;
    }


    function f2(code) {
        document.getElementById('cancel_form').setAttribute("action",'${pageContext.request.contextPath}/d/'+code+'/cancel');
    }
    function f3(code) {
        document.getElementById('cancel_form').setAttribute("action",'${pageContext.request.contextPath}/d/'+code+'/up');
    }
    function  cancel() {
...
<script>
    $("#submit-all-id").on("click",function () {
        ...
        if(idString==''){
            ...
            var url = '${pageContext.request.contextPath}/d/'+idString+'/up_group';
            window.location.href=url;
        }
    });
</script>
